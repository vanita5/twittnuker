...
public class HtmlBuilder {
	...
	public String build() {
		...
		for (int i = 0; i < linksSize; i++) {
			...
			if (start >= 0 && start <= end && end <= codePointsLength) {
				builder.append(!isEmpty(spec.display) ? spec.display_is_html ? spec.display : HtmlEscapeHelper.toHtml(spec.display)
						: spec.link);
			}
			builder.append("</a>");
	...
	private void appendSource(final StringBuilder builder, final int start, final int end) {
		if (sourceIsEscaped == shouldReEscape) {
			...
			builder.append(HtmlEscapeHelper.escape(subString(start, end)));
		} else {
			builder.append(HtmlEscapeHelper.unescape(subString(start, end)));
		}
	}

	private String escapeSource(final String source) {
		if (sourceIsEscaped == shouldReEscape) return source;
		return shouldReEscape ? HtmlEscapeHelper.escape(source) : HtmlEscapeHelper.unescape(source);
	}
